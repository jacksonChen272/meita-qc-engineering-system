...
(async () => {
  ...
  if (await page.locator("[data-online-template]").count() !== 2) throw new Error("線上版沒有提供兩個 QC 母版");
  const setupText = await page.locator(".online-setup-panel").innerText();
  ...
  console.log(JSON.stringify({ templates: 2, selectedTemplate: "nutrition", onlineSetup: true, parsed: true, sheets: 8, errors }, null, 2));
  await browser.close();
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
