...
(async () => {
  ...
  await page.setInputFiles("[data-online-qc]", path.join(__dirname, "..", "input", "qc-template.docx"));
  ...
  console.log(JSON.stringify({ onlineSetup: true, parsed: true, sheets: 8, errors }, null, 2));
  await browser.close();
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
